################################################################################
##
## Image Tools by Feniks (feniksdev.itch.io / feniksdev.com)
##
################################################################################
...
default sprt.xadj = NoInertiaAdjustment(range=1.0, value=0.5)
default sprt.yadj = NoInertiaAdjustment(range=1.0, value=0.5)
...
init -90 python in sprt:
    ...
    def construct_checkbox(border_color, inside="#000d", width=5,
        ...
        """
        if checked:
            s = Solid(sprt.WHITE, align=(0.5, 0.5), xysize=(int(box_size[0]//3),
                int(box_size[1]//3)))
        else:
            s = Solid("#0000", align=(0.5, 0.5),
                xysize=(int(box_size[0]//3), int(box_size[1]//3)))
        return Window(
            s,
...
screen hamburger_menu():
    ...
    showif show_options: # To animate
...
init python:
    ...
    build.classify("**image_tool_common.rpyc", None)
################################################################################